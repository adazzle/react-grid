  ...
  for (i = 0, len = columns.length; i < len; i++) {
    ...
    if (column.width) {
      if (/^([0-9]+)%$/.exec(column.width)) {
        if(metrics.totalWidth === null) {
          //this is the case on initial load
          //we need to know what we are doing a % of, so assume the screen width
          //TODO this shoudl probably be something we require you to pass in?
          metrics.totalWidth = getWindowSize().width;
        }
        column.width = Math.floor(parseInt(column.width, 10) / 100 * metrics.totalWidth);
      }
...
var Mixin = {
  ...
  gridWidth: function() {
    return this.isMounted() ? this.getDOMNode().offsetWidth - 2 : 0;
  },
  ...
  getColumnMetrics: function(props, initial) {
    var totalWidth = initial ? null : this.gridWidth();
    return {
